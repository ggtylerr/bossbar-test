...
public class CommandPlayersAdd implements TabExecutor {
    @Override
        ...
        if (sender instanceof Player) {
            ...
            if (args.length == 2) {
                ...
                if (success.size() == 0) {
                    ...
                    msg = new TextComponent("Successfully added the player '" + success.get(0) + "' to the bar ");
                    msg.setColor(net.md_5.bungee.api.ChatColor.GREEN);
                    msg.addExtra(GenComponent.bar(bar, key));
                    player.spigot().sendMessage(msg);
                }
            } else {
                TextComponent msg = new TextComponent("Players successfully added to ");
                msg.setColor(net.md_5.bungee.api.ChatColor.GREEN);
                msg.addExtra(GenComponent.bar(bar, key));
                msg.addExtra(": ");
                if (success.size() == 0) {
                ...
                TextComponent failMsg = new TextComponent("\nPlayers not added (they probably haven't joined before): ");
            ...
            if (args.length == 2) {
                if (success.size() == 0) {
                    ...
                    sender.sendMessage(ChatColor.GREEN + "Successfully added the player '" + success.get(0) + "' to the bar " + bar.getTitle());
                }
            } else {
                ...
                sender.sendMessage(
                        ChatColor.GREEN + "Players successfully added to " + bar.getTitle() + ": " +
                                successStr + "\n" +
                                ChatColor.RED + "Players not added (they probably don't exist): " +
                                failStr
                );
            }
        }
