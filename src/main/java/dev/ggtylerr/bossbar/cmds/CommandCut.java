...
public class CommandCut implements TabExecutor {
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (oldBar == null) {
            sender.sendMessage(ChatColor.RED + "The bar you're trying to cut doesn't exist!");
            return false;
        }
        NamespacedKey newKey = new NamespacedKey(Main.getPlugin(Main.class), args[1]);
        if (Bukkit.getBossBar(newKey) != null) {
            sender.sendMessage(ChatColor.RED + "The bar you're trying to cut to already exists!");
            return false;
        }
        ...
        newBar.setVisible(oldBar.isVisible());
        List<Player> players = oldBar.getPlayers();
        for (Player p : players) {
            newBar.addPlayer(p);
        }
        TextComponent oldComp = GenComponent.bar(oldBar, oldKey);
        String oldTitle = oldBar.getTitle();
        ...
        if (sender instanceof Player) {
            ...
            msg.addExtra(oldComp);
            msg.addExtra(" successfully cut to ");
            msg.addExtra(GenComponent.bar(newBar, newKey));
            player.spigot().sendMessage(msg);
        }
        else sender.sendMessage(ChatColor.GREEN + "Bar '" + oldTitle + "' successfully cut to '" + newBar.getTitle() + "'");
        return true;
    }
    @Override
