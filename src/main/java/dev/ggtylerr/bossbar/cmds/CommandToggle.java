...
public class CommandToggle implements TabExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (bar == null) {
            sender.sendMessage(ChatColor.RED + "The bar you're trying to hide doesn't exist!");
            return false;
        }
        bar.setVisible(!bar.isVisible());
        String visString = bar.isVisible() ? "visible" : "hidden";
        if (sender instanceof Player) {
            ...
            msg.addExtra(" has been set to be " + visString);
            player.spigot().sendMessage(msg);
        }
        else sender.sendMessage(ChatColor.GREEN + "Bar '" + bar.getTitle() + "' has been set to be " + visString);
        return true;
    }
    @Override
