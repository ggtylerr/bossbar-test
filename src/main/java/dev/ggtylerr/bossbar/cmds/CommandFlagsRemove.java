...
public class CommandFlagsRemove implements TabExecutor {
    @Override
        ...
        if (sender instanceof Player) {
            ...
            if (args.length == 2) {
                ...
                if (success.size() == 0) {
                    ...
                    msg = new TextComponent("Successfully removed the flag '" + success.get(0) + "' from the bar ");
                    msg.setColor(net.md_5.bungee.api.ChatColor.GREEN);
                    msg.addExtra(GenComponent.bar(bar, key));
                    player.spigot().sendMessage(msg);
                }
            } else {
                TextComponent msg = new TextComponent("Flags successfully removed from ");
                msg.setColor(net.md_5.bungee.api.ChatColor.GREEN);
                msg.addExtra(GenComponent.bar(bar, key));
                msg.addExtra(": ");
                if (success.size() == 0) {
                ...
                TextComponent failMsg = new TextComponent("\nFlags not removed (they probably don't exist): ");
            ...
            if (args.length == 2) {
                if (success.size() == 0) {
                    ...
                    sender.sendMessage(ChatColor.GREEN + "Successfully removed the flag '" + success.get(0) + "' from the bar " + bar.getTitle());
                }
            } else {
                ...
                sender.sendMessage(
                        ChatColor.GREEN + "Flags successfully removed from " + bar.getTitle() + ": " +
                                successStr + "\n" +
                                ChatColor.RED + "Flags not removed (they probably don't exist): " +
                                failStr
                );
            }
        }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        ...
        if (args.length > 1) return Arrays.asList("fog", "dark", "music", "all");
        return null;
    }
}
